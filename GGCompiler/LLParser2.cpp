...
/// module.
bool LLParser2::ValidateEndOfModule() {
...
bool LLParser2::ResolveForwardRefBlockAddresses(Function *TheFn,
...
bool LLParser2::ParseTopLevelEntities() {
  while (1) {
    switch (Lex.getKind()) {
...
bool LLParser2::ParseTargetDefinition() {
...
bool LLParser2::ParseUnnamedType() {
...
///   ::= 'constant'
///   ::= 'global'
bool LLParser2::ParseGlobalType(bool &IsConstant) {
...
///                                                     ...   -> global variable
bool LLParser2::ParseUnnamedGlobal() {
...
///   GlobalVar '=' OptionalLinkage OptionalVisibility OptionalDLLStorageClass
///                                                     ...   -> global variable
bool LLParser2::ParseNamedGlobal() {
...
// MDString:
//   ::= '!' STRINGCONSTANT
bool LLParser2::ParseMDString(MDString *&Result) {
  std::string Str;
  if (ParseStringConstant(Str)) return true;
...
bool LLParser2::ParseMDNodeID(MDNode *&Result, unsigned &SlotNo) {
  // !{ ..., !42, ... }
...
bool LLParser2::ParseMDNodeID(MDNode *&Result) {
  // !{ ..., !42, ... }
  unsigned MID = 0;
...
/// ParseNamedMetadata:
///   !foo = !{ !1, !2 }
bool LLParser2::ParseNamedMetadata() {
...
///   !42 = !{...}
bool LLParser2::ParseStandaloneMetadata() {
...
///
bool LLParser2::ParseAlias(const std::string &Name, LocTy NameLoc,
...
/// already.
///
bool LLParser2::ParseGlobal(const std::string &Name, LocTy NameLoc,
...
bool LLParser2::ParseUnnamedAttrGrp() {
...
///   ::= <attr> | <attr> '=' <value>
bool LLParser2::ParseFnAttributeValuePairs(AttrBuilder &B,
...
GlobalValue *LLParser2::GetGlobalVal(const std::string &Name, Type *Ty,
                                    LocTy Loc) {
...
GlobalValue *LLParser2::GetGlobalVal(unsigned ID, Type *Ty, LocTy Loc) {
  PointerType *PTy = dyn_cast<PointerType>(Ty);
  if (!PTy) {
    Error(Loc, "global variable reference must have pointer type");
    return nullptr;
...
Comdat *LLParser2::getComdat(const std::string &Name, LocTy Loc) {
...
bool LLParser2::ParseToken(lltok::Kind T, const char *ErrMsg) {
  if (Lex.getKind() != T)
...
/// ParseStringConstant
///   ::= StringConstant
bool LLParser2::ParseStringConstant(std::string &Result) {
...
/// ParseUInt32
///   ::= uint32
bool LLParser2::ParseUInt32(unsigned &Val) {
...
/// ParseUInt64
///   ::= uint64
bool LLParser2::ParseUInt64(uint64_t &Val) {
  if (Lex.getKind() != lltok::APSInt || Lex.getAPSIntVal().isSigned())
    return TokError("expected integer");
...
///   := 'initialexec'
///   := 'localexec'
bool LLParser2::ParseTLSModel(GlobalVariable::ThreadLocalMode &TLM) {
  switch (Lex.getKind()) {
    default:
...
bool LLParser2::ParseOptionalThreadLocal(GlobalVariable::ThreadLocalMode &TLM) {
...
///   := /*empty*/
///   := 'addrspace' '(' uint32 ')'
bool LLParser2::ParseOptionalAddrSpace(unsigned &AddrSpace) {
  AddrSpace = 0;
...
bool LLParser2::ParseOptionalParamAttrs(AttrBuilder &B) {
  bool HaveError = false;
...
bool LLParser2::ParseOptionalReturnAttrs(AttrBuilder &B) {
  bool HaveError = false;
...
///     ::= 'linker_private'
///     ::= 'linker_private_weak'
bool LLParser2::ParseOptionalLinkage(unsigned &Res, bool &HasLinkage) {
  HasLinkage = false;
  switch (Lex.getKind()) {
...
///   ::= 'protected'
///
bool LLParser2::ParseOptionalVisibility(unsigned &Res) {
  switch (Lex.getKind()) {
...
///   ::= 'dllexport'
///
bool LLParser2::ParseOptionalDLLStorageClass(unsigned &Res) {
  switch (Lex.getKind()) {
...
///   ::= 'cc' UINT
///
bool LLParser2::ParseOptionalCallingConv(CallingConv::ID &CC) {
  switch (Lex.getKind()) {
...
///   ::= !dbg !42 (',' !dbg !57)*
bool LLParser2::ParseInstructionMetadata(Instruction *Inst,
                                        PerFunctionState *PFS) {
  do {
...
///   ::= /* empty */
///   ::= 'align' 4
bool LLParser2::ParseOptionalAlignment(unsigned &Alignment) {
  Alignment = 0;
...
///   ::= /* empty */
///   ::= 'dereferenceable' '(' 4 ')'
bool LLParser2::ParseOptionalDereferenceableBytes(uint64_t &Bytes) {
  Bytes = 0;
...
/// end.
bool LLParser2::ParseOptionalCommaAlign(unsigned &Alignment,
                                       bool &AteExtraComma) {
  AteExtraComma = false;
  while (EatIfPresent(lltok::comma)) {
...
bool LLParser2::ParseScopeAndOrdering(bool isAtomic, SynchronizationScope &Scope,
...
bool LLParser2::ParseOrdering(AtomicOrdering &Ordering) {
  switch (Lex.getKind()) {
...
///   ::= /* empty */
///   ::= 'alignstack' '(' 4 ')'
bool LLParser2::ParseOptionalStackAlignment(unsigned &Alignment) {
  Alignment = 0;
...
///    ::=  (',' uint32)+
///
bool LLParser2::ParseIndexList(SmallVectorImpl<unsigned> &Indices,
                              bool &AteExtraComma) {
  AteExtraComma = false;
...
bool LLParser2::ParseType(Type *&Result, bool AllowVoid) {
...
bool LLParser2::ParseParameterList(SmallVectorImpl<ParamInfo> &ArgList,
                                  PerFunctionState &PFS) {
...
///   ::= ArgType (',' ArgType)*
///
bool LLParser2::ParseArgumentList(SmallVectorImpl<ArgInfo> &ArgList,
                                 bool &isVarArg){
  isVarArg = false;
...
bool LLParser2::ParseFunctionType(Type *&Result) {
  assert(Lex.getKind() == lltok::lparen);
...
/// other structs.
bool LLParser2::ParseAnonStructType(Type *&Result, bool Packed) {
...
bool LLParser2::ParseStructDefinition(SMLoc TypeLoc, StringRef Name,
...
///     ::= '<' '{' '}' '>'
///     ::= '<' '{' Type (',' Type)* '}' '>'
bool LLParser2::ParseStructBody(SmallVectorImpl<Type*> &Body) {
...
///     ::= '[' APSINTVAL 'x' Types ']'
///     ::= '<' APSINTVAL 'x' Types '>'
bool LLParser2::ParseArrayVectorType(Type *&Result, bool isVector) {
  if (Lex.getKind() != lltok::APSInt || Lex.getAPSIntVal().isSigned() ||
...
LLParser2::PerFunctionState::PerFunctionState(LLParser2 &p, Function &f,
                                             int functionNumber)
...
LLParser2::PerFunctionState::~PerFunctionState() {
...
bool LLParser2::PerFunctionState::FinishFunction() {
...
/// forward reference record if needed.  This can return null if the value
/// exists but does not have the right type.
Value *LLParser2::PerFunctionState::GetVal(const std::string &Name,
                                          Type *Ty, LocTy Loc) {
  // Look this name up in the normal function symbol table.
...
Value *LLParser2::PerFunctionState::GetVal(unsigned ID, Type *Ty,
                                          LocTy Loc) {
  // Look this name up in the normal function symbol table.
...
bool LLParser2::PerFunctionState::SetInstName(int NameID,
...
BasicBlock *LLParser2::PerFunctionState::GetBB(const std::string &Name,
                                              LocTy Loc) {
  return cast_or_null<BasicBlock>(GetVal(Name,
                                        Type::getLabelTy(F.getContext()), Loc));
}

BasicBlock *LLParser2::PerFunctionState::GetBB(unsigned ID, LocTy Loc) {
...
BasicBlock *LLParser2::PerFunctionState::DefineBB(const std::string &Name,
                                                 LocTy Loc) {
  BasicBlock *BB;
  if (Name.empty())
...
bool LLParser2::ParseValID(ValID &ID, PerFunctionState *PFS) {
  ID.Loc = Lex.getLoc();
  switch (Lex.getKind()) {
...
bool LLParser2::ParseGlobalValue(Type *Ty, Constant *&C) {
  C = nullptr;
  ValID ID;
  Value *V = nullptr;
...
bool LLParser2::ParseGlobalTypeAndValue(Constant *&V) {
  Type *Ty = nullptr;
  return ParseType(Ty) ||
         ParseGlobalValue(Ty, V);
}

bool LLParser2::parseOptionalComdat(Comdat *&C) {
  C = nullptr;
...
bool LLParser2::ParseGlobalValueVector(SmallVectorImpl<Constant*> &Elts) {
  // Empty list.
  if (Lex.getKind() == lltok::rbrace ||
...
bool LLParser2::ParseMetadataListValue(ValID &ID, PerFunctionState *PFS) {
  assert(Lex.getKind() == lltok::lbrace);
  Lex.Lex();
...
///  ::= !{...}
///  ::= !"string"
bool LLParser2::ParseMetadataValue(ValID &ID, PerFunctionState *PFS) {
  assert(Lex.getKind() == lltok::exclaim);
  Lex.Lex();
...
bool LLParser2::ConvertValIDToValue(Type *Ty, ValID &ID, Value *&V,
                                   PerFunctionState *PFS) {
  if (Ty->isFunctionTy())
...
bool LLParser2::ParseValue(Type *Ty, Value *&V, PerFunctionState *PFS) {
  V = nullptr;
  ValID ID;
...
bool LLParser2::ParseTypeAndValue(Value *&V, PerFunctionState *PFS) {
  Type *Ty = nullptr;
  return ParseType(Ty) ||
         ParseValue(Ty, V, PFS);
}

bool LLParser2::ParseTypeAndBasicBlock(BasicBlock *&BB, LocTy &Loc,
                                      PerFunctionState &PFS) {
  Value *V;
  Loc = Lex.getLoc();
...
bool LLParser2::ParseFunctionHeader(Function *&Fn, bool isDefine) {
  // Parse the linkage.
  LocTy LinkageLoc = Lex.getLoc();
  unsigned Linkage;
...
///   ::= '{' BasicBlock+ '}'
///
bool LLParser2::ParseFunctionBody(Function &Fn) {
  if (Lex.getKind() != lltok::lbrace)
...
bool LLParser2::ParseBasicBlock(PerFunctionState &PFS) {
  // If this basic block starts out with a name, remember it.
  std::string Name;
  LocTy NameLoc = Lex.getLoc();
  if (Lex.getKind() == lltok::LabelStr) {
...
bool LLParser2::ParseBasicBlock2(PerFunctionState &PFS, BasicBlock *BB) {
  std::string NameStr;
...
///
int LLParser2::ParseInstruction(Instruction *&Inst, BasicBlock *BB,
                               PerFunctionState &PFS) {
  lltok::Kind Token = Lex.getKind();
  if (Token == lltok::Eof)
...
bool LLParser2::ParseCmpPredicate(unsigned &P, unsigned Opc) {
  if (Opc == Instruction::FCmp) {
    switch (Lex.getKind()) {
...
bool LLParser2::ParseRet(Instruction *&Inst, BasicBlock *BB,
                        PerFunctionState &PFS) {
  SMLoc TypeLoc = Lex.getLoc();
  Type *Ty = nullptr;
...
bool LLParser2::ParseBr(Instruction *&Inst, PerFunctionState &PFS) {
  LocTy Loc, Loc2;
  Value *Op0;
  BasicBlock *Op1, *Op2;
...
///  JumpTable
///    ::= (TypeAndValue ',' TypeAndValue)*
bool LLParser2::ParseSwitch(Instruction *&Inst, PerFunctionState &PFS) {
  LocTy CondLoc, BBLoc;
  Value *Cond;
...
bool LLParser2::ParseIndirectBr(Instruction *&Inst, PerFunctionState &PFS) {
  LocTy AddrLoc;
  Value *Address;
...
bool LLParser2::ParseInvoke(Instruction *&Inst, PerFunctionState &PFS) {
...
/// ParseResume
///   ::= 'resume' TypeAndValue
bool LLParser2::ParseResume(Instruction *&Inst, PerFunctionState &PFS) {
...
bool LLParser2::ParseArithmetic(Instruction *&Inst, PerFunctionState &PFS,
...
/// ParseLogical
///  ::= ArithmeticOps TypeAndValue ',' Value {
bool LLParser2::ParseLogical(Instruction *&Inst, PerFunctionState &PFS,
                            unsigned Opc) {
  LocTy Loc; Value *LHS, *RHS;
  if (ParseTypeAndValue(LHS, Loc, PFS) ||
...
bool LLParser2::ParseCompare(Instruction *&Inst, PerFunctionState &PFS,
                            unsigned Opc) {
...
bool LLParser2::ParseCast(Instruction *&Inst, PerFunctionState &PFS,
                         unsigned Opc) {
  LocTy Loc;
  Value *Op;
  Type *DestTy = nullptr;
...
bool LLParser2::ParseSelect(Instruction *&Inst, PerFunctionState &PFS) {
  LocTy Loc;
  Value *Op0, *Op1, *Op2;
...
bool LLParser2::ParseVA_Arg(Instruction *&Inst, PerFunctionState &PFS) {
  Value *Op;
  Type *EltTy = nullptr;
  LocTy TypeLoc;
...
bool LLParser2::ParseExtractElement(Instruction *&Inst, PerFunctionState &PFS) {
  LocTy Loc;
  Value *Op0, *Op1;
  if (ParseTypeAndValue(Op0, Loc, PFS) ||
...
bool LLParser2::ParseInsertElement(Instruction *&Inst, PerFunctionState &PFS) {
  LocTy Loc;
  Value *Op0, *Op1, *Op2;
  if (ParseTypeAndValue(Op0, Loc, PFS) ||
...
bool LLParser2::ParseShuffleVector(Instruction *&Inst, PerFunctionState &PFS) {
  LocTy Loc;
  Value *Op0, *Op1, *Op2;
  if (ParseTypeAndValue(Op0, Loc, PFS) ||
...
int LLParser2::ParsePHI(Instruction *&Inst, PerFunctionState &PFS) {
...
bool LLParser2::ParseLandingPad(Instruction *&Inst, PerFunctionState &PFS) {
...
///       ParameterList OptionalAttrs
bool LLParser2::ParseCall(Instruction *&Inst, PerFunctionState &PFS,
...
int LLParser2::ParseAlloc(Instruction *&Inst, PerFunctionState &PFS) {
  Value *Size = nullptr;
  LocTy SizeLoc;
...
int LLParser2::ParseLoad(Instruction *&Inst, PerFunctionState &PFS) {
  Value *Val; LocTy Loc;
  unsigned Alignment = 0;
  bool AteExtraComma = false;
  bool isAtomic = false;
...
///       'singlethread'? AtomicOrdering (',' 'align' i32)?
int LLParser2::ParseStore(Instruction *&Inst, PerFunctionState &PFS) {
...
int LLParser2::ParseCmpXchg(Instruction *&Inst, PerFunctionState &PFS) {
...
int LLParser2::ParseAtomicRMW(Instruction *&Inst, PerFunctionState &PFS) {
...
int LLParser2::ParseFence(Instruction *&Inst, PerFunctionState &PFS) {
  AtomicOrdering Ordering = NotAtomic;
  SynchronizationScope Scope = CrossThread;
...
int LLParser2::ParseGetElementPtr(Instruction *&Inst, PerFunctionState &PFS) {
  Value *Ptr = nullptr;
  Value *Val = nullptr;
  LocTy Loc, EltLoc;
...
int LLParser2::ParseExtractValue(Instruction *&Inst, PerFunctionState &PFS) {
  Value *Val; LocTy Loc;
  SmallVector<unsigned, 4> Indices;
  bool AteExtraComma;
  if (ParseTypeAndValue(Val, Loc, PFS) ||
...
int LLParser2::ParseInsertValue(Instruction *&Inst, PerFunctionState &PFS) {
...
/// Element
///   ::= 'null' | TypeAndValue
bool LLParser2::ParseMDNodeVector(SmallVectorImpl<Value*> &Elts,
                                 PerFunctionState *PFS) {
